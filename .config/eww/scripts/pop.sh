#!/bin/bash

monitor=$(hyprctl activewindow | grep 'monitor' | awk '{print $2}')

calendar() {
LOCK_FILE="$HOME/.cache/eww-calendar.lock"

run() {
    eww -c $HOME/.config/eww open --screen "$monitor" calendar
}

# Open widgets
if [[ ! -f "$LOCK_FILE" ]]; then
    eww -c $HOME/.config/eww close system audio_ctl sidebar
    touch "$LOCK_FILE"
    run && echo "ok good!"
else
    eww -c $HOME/.config/eww close calendar
    rm "$LOCK_FILE" && echo "closed"
fi
}

audio() {
LOCK_FILE_AUDIO="$HOME/.cache/eww-audio.lock"

run() {
    eww -c $HOME/.config/eww open --screen "$monitor" audio_ctl 
}

# Open widgets
if [[ ! -f "$LOCK_FILE_AUDIO" ]]; then
    eww -c $HOME/.config/eww close system calendar sidebar
    touch "$LOCK_FILE_AUDIO"
    run && echo "ok good!"
else
    eww -c $HOME/.config/eww close audio_ctl
    rm "$LOCK_FILE_AUDIO" && echo "closed"
fi
}

system() {
LOCK_FILE_MEM="$HOME/.cache/eww-system.lock"

run() {
    eww -c $HOME/.config/eww open --screen "$monitor" system 
}

# Open widgets
if [[ ! -f "$LOCK_FILE_MEM" ]]; then
    eww -c $HOME/.config/eww close calendar audio_ctl sidebar
    touch "$LOCK_FILE_MEM"
    run && echo "ok good!"
else
    eww -c $HOME/.config/eww close system 
    rm "$LOCK_FILE_MEM" && echo "closed"
fi
}

sidebar() {
LOCK_FILE_SIDE="$HOME/.cache/eww-sidebar.lock"

run() {
    eww -c $HOME/.config/eww open-many sidebar:s weather:w timer:t search:se options:o --arg s:screen="$monitor" --arg w:screen="$monitor" --arg t:screen="$monitor" --arg se:screen="$monitor" --arg o:screen="$monitor"
}

# Open widgets
if [[ ! -f "$LOCK_FILE_SIDE" ]]; then
    eww -c $HOME/.config/eww close system calendar audio_ctl
    touch "$LOCK_FILE_SIDE"
    run && echo "ok good!"
else
    eww close s 
    eww close w
    eww close t
    eww close se
    eww close o
    rm "$LOCK_FILE_SIDE" && echo "closed"
fi
}

hyprlock(){
    hyprlock
    eww close s
    eww close w
    eww close t
    eww close se
    eww close o
}
"$@"
